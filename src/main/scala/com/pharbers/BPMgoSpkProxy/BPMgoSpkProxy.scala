...
object BPMgoSpkProxyImpl {
    ...
    def loadDataFromMgo2Spark(
        ...
        cal_data.write.parquet("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + job_id + "/input/" + "cal_data")
//        cal_data.write.json("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + job_id + "/json/" + "cal_data")
        ...
        cal_comp.write.parquet("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + job_id + "/input/" + "cal_comp")
//        cal_comp.write.json("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + job_id + "/json/" + "cal_comp")
    ...
    def loadDataFromSpark2Mgo(
        ...
        val spark = SparkSession.builder().config(conf).getOrCreate()
        val df_cal = spark.read.parquet("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + jobId + "/output/" + "cal_report")
        df_cal.write
        ...
        val df_comp = spark.read.parquet("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + jobId + "/output/" + "competitor")
        df_comp.write
            .format("com.mongodb.spark.sql.DefaultSource")
            .option("spark.mongodb.output.uri", s"${mongoUri + destDatabase}")
        ...
        val df_summary = spark.read.parquet("hdfs://192.168.100.14:8020/tmtest0831/jobs/" + jobId + "/output/" + "summary")
        df_summary.write
            .format("com.mongodb.spark.sql.DefaultSource")
            .option("spark.mongodb.output.uri", s"${mongoUri + destDatabase}")
